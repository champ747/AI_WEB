  ...
  const fetchFavorites = async () => {
    ...
    try {
      const response = await fetch(
        'https://port-0-back-m341pqyi646021b2.sel4.cloudtype.app/favorites',
        {
          method: 'GET',
          headers: {
      ...
      console.log('찜 목록 조회 결과:', data);
      return data.favorites.map((favorite) => favorite.cafe_id);
    } catch (error) {
  ...
  useEffect(() => {
    const fetchCafeInfo = async () => {
      try {
        ...
        // 찜 목록 조회 및 상태 업데이트
        const favoriteCafes = await fetchFavorites();
        setIsLiked(favoriteCafes.includes(cafeId));
      } catch (error) {
