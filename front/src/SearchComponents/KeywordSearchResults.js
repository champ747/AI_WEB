...
const KeywordSearchResults = () => {
  ...
  const fetchSearchResults = async (page = 1, order = "relevance") => {
    ...
    const baseUrl = `https://port-0-back-m341pqyi646021b2.sel4.cloudtype.app/cafes/search?category=${encodeURIComponent(
      ...
      await new Promise((resolve, reject) =>
        navigator.geolocation.getCurrentPosition(
          (position) => {
            ...
            url += `&sortByProximity=true&latitude=${latitude}&longitude=${longitude}`;
            resolve();
          },
          (error) => {
        ...
        try {
          const response = await fetch(
            `https://port-0-back-m341pqyi646021b2.sel4.cloudtype.app/cafes/search?category=${encodeURIComponent(
              requestData.categories.join(',')
            )}&limit=10&page=${currentPage}&sortByProximity=true&latitude=${latitude}&longitude=${longitude}`
          );
  
          if (!response.ok) {
            throw new Error('가까운 순 정렬을 불러오는 데 실패했습니다.');
          }
  
          const data = await response.json();
          console.log('가까운 순 정렬 데이터:', data);
  
          setSearchResults(data.cafes);
          setTotalPages(data.totalPages);
        } catch (error) {
          console.error('가까운 순 정렬 중 오류 발생:', error);
          setError('가까운 순 정렬 중 오류가 발생했습니다.');
        }
      },
      (error) => {
        console.error('위치 정보 가져오기 오류:', error);
