...
const KeywordSearchPage = () => {
  ...
  const [selectedKeywords, setSelectedKeywords] = useState([]); // 선택된 키워드 하나만 저장
  ...
  const toggleKeyword = (keyword) => {
    // 선택된 키워드가 이미 있다면 해제, 없다면 해당 키워드만 선택
    setSelectedKeywords((prev) => (prev[0] === keyword ? [] : [keyword]));
  };
